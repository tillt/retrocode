...
CMFMProperty::CMFMProperty(void) 
{
}
...
void CMFMProperty::InitFromContent(LPCTSTR pszPath,unsigned int nSize,CMobileContent *pm)
{
	CMobileProperty::InitFromContent(pszPath,nSize,pm);
	CMFMFile *pMfm=(CMFMFile *)pm;
	ASSERT(pMfm);
	if (pMfm)
	{
		SetProperty_long(prstrSubFormat,"OKI ADPCM","id","1");
